...
public class EmailService {
    ...
    public String composeEmail(Collection<Article> articles){
        String email = articles.stream().map(article -> (
            "<div>" +
                "<h4>" + article.getHeader() + "</h4>" +
                "<h5>" + article.getSubheader() + "</h4>" +
                "<p>" + article.getText() + "</p>" +
                "<img src=\"" + article.getPhoto() + "\" style=\"max-width:250px;width:100%\">" +
    ...
    @Async
    public void sendArticles(String to, Collection<Article> articles){
